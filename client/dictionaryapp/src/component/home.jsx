...
function HomePage() {
  ...
  const [nemail, setNemail] = useState('');   //for newsletter
  const [cData, setCdata] = useState({name:'',email:'',message:''});
  ...
  // bookamark
  const handleBookmark = async(word, def, syns, ants) => {
    ...
    const res = await fetch('/book',{
      ...
      body: JSON.stringify({word, def, syns, ants})
    });
    ...
    if(!data){
      ...
      toast.success("Word added to bookmark section");
    }
  };
  ...
  const fetchCdata = async() => {
    try{
      ...
      const data = await res.json();
      setCdata({...cData, name:data.name, email: data.email});
  ...
  // storing and sending contact us messages
  const handleInput =(e) =>{
  ...
  const contactForm = async (e) => {
    ...
    if(!token){
      ...
      const {name, email, message} = cData;
    ...
    const data = await res.json();
    if(!data || cData.message === ''){
      toast.error("Message not sent");
    } else{
      toast.success("Message sent");
      setCdata({...cData,message: ""});
    }
    }
          ...
          <div className="col-md-4">
            ...
            <form>
              <div className="form-group">
                <input type="email" className="form-control" placeholder="Enter your email" value={nemail} onChange={(e) => setNemail(e.target.value)} />
              </div>
              <button type="submit" className="subscribe-button" onClick={handleNewsletter}>Subscribe</button>
            </form>
          </div>
          <div className="col-md-4">
            <h3>Contact</h3>
            <form>
              <div className="form-group">
                <input type="text" className="form-control" placeholder="Your Name" name= "name" value={cData.name} onChange={handleInput}/>
              </div>
              <div className="form-group">
                <input type="email" className="form-control" placeholder="Your Email" name= "email" value={cData.email} onChange={handleInput}/>
              </div>
              <div className="form-group">
                <textarea className="form-control" placeholder="Your Message" name= "message" value={cData.message} onChange={handleInput}></textarea>
              </div>
